...
class DailyReportController extends GetxController{
  ...
  Future getBanks() async {
    ...
    ReportsRepository().getAllGlPayGallaries(AllInvoicesGAllariesRequest(
        ...
        invInventoryDtoList: selectedGalleries.map((e) => DtoList(id: e.id)).toList(),
    ),
      onSuccess: (data){
    ...
    ReportsRepository().getSafeAccountStatement(request,
      onSuccess: (data){
        ...
        for (var element in treasuries) {
          treasuriesTotal(treasuriesTotal.value + (element.statements.first.totalDebit ?? 0));
        }
        treasuries.refresh();
  ...
  getGalleryExpensesList(){
    ...
    GalleryExpensesRepository().getExpensesList(request,
     onComplete: i == selectedGalleries.length -1 ? () => isLoading(false) : null,
      onError: (e) => showPopupText(text: e.toString()),
  ...
  selectNewBank(List<String> values) {
    if (!values.contains("تحديد الكل") && selectedBanks.any((element) => element.name == "تحديد الكل")) {
      ...
      selectedBanks.assignAll(banks.where((element) => values.contains(element.name)));
